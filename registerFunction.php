...
if (empty($_POST['name']) || empty($_POST['surname']) || empty($_POST['door_number']) || empty($_POST['phone_number']) || empty($_POST['email'])  || empty($_POST['people']) || empty($_POST['deposit']) || empty($_POST['user_name']) ) {
...
$iQuery = "INSERT Into customer (name, surname, door_number,phone_number,email,people,deposit,user_name) values(?,?,?, ?, ?,?,?,?)";
...
if($rnum==0) { //if true, insert new data
    	  ...
    	  $stmt->bind_param("ssssssss", $name, $surname, $door_number,$phone_number,$email,$people,$deposit,$user_name);
          if($stmt->execute()) {
